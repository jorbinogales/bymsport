<?php
/**
 * Plugin Name: Home Category List Widget
 */

add_action( 'widgets_init', 'mvp_home_catlist_load_widgets' );

function mvp_home_catlist_load_widgets() {
	register_widget( 'mvp_home_catlist_widget' );
}

class mvp_home_catlist_widget extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'mvp_home_catlist_widget', 'description' => esc_html__('A widget that displays a list of 4 posts from a category of your choice in the homepage widget area.', 'the-league') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'mvp_home_catlist_widget' );

		/* Create the widget. */
		parent::__construct( 'mvp_home_catlist_widget', esc_html__('The League: Home Cat List Widget', 'the-league'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<?php if ( $title ) { ?>
			<h4 class="mvp-sec-head"><span class="mvp-sec-head"><?php echo $title; ?></span><span class="mvp-widget-head-link"><a href="<?php echo get_category_link($categories); ?>"><?php esc_html_e( 'More', 'the-league' ); ?> <?php echo esc_html( $title ); ?> <i class="fa fa-chevron-right" aria-hidden="true"></i></a></span></h4>
		<?php } ?>
		<div class="mvp-widget-list-wrap left relative">
			<?php $recent = new WP_Query(array( 'cat' => $categories, 'posts_per_page' => '4' )); while($recent->have_posts()) : $recent->the_post(); ?>
